...
from actqkv.utils.extract_question import extract_question_id
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if(self.model_provider not in ["OpenAI", "Anthropic"]):
            ...
            if self.conv_type == 'mistral-inst':
                from actqkv.models.modeling_mistral import MistralForCausalLM
                ...
                from actqkv.models.modeling_llama import LlamaForCausalLM
            ...
            if self.config.type in ['qllm','actqkv_dynamic_only','inf-llm', 'stream-llm', 'infinite-lm']:
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if(self.model_provider in ["OpenAI", "Anthropic"]):
            ...
            if self.config.type in ['qllm','actqkv_dynamic_only']:
            ...
            if self.config.type in ['qllm', 'inf-llm', 'actqkv_dynamic_only','stream-llm', 'infinite-lm']:
                kwargs['chunk_size'] = self.chunk_size

            with torch.no_grad():
                output_ids = self.model_to_test.generate(**kwargs)
                if self.config.type not in ['qllm', 'actqkv_dynamic_only','inf-llm', 'stream-llm', 'infinite-lm']:
