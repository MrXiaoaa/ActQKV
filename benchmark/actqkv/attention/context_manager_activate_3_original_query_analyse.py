...
class VectorTensor:
    ...
    def get_topk(self, tensor: torch.Tensor, avctivated_weight:torch.Tensor, activate_question_weight:Optional[torch.Tensor],topk):  # inner product
        ...
        original_tensor = tensor.sum(dim=0, keepdim=False)
        ...
        #logits_c = torch.matmul(self.data[:self.length], tensor[:, None]).squeeze(dim=-1) 
        # cosine similarity
        original_logits_c = torch.matmul(F.normalize(self.data[:self.length], dim=-1), F.normalize(original_tensor, dim=-1)[:, None]).squeeze(dim=-1)
        ...
        if activate_question_weight is not None and inference_step < layer_num:
            ...
            data_to_save = {
                ...
                'original_logits_c':original_logits_c.cpu().tolist(),
            ...
            file_name = f'./result/analyse/activate_score.json'
