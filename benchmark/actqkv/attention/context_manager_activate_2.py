...
class VectorTensor:
    ...
    def get_topk(self, tensor: torch.Tensor, topk): # inner product
        ...
        H_min = 0
        H_max = math.log(len(scores)+1)
        new_topk = self.adjust_tok_K(len(scores), entropy_scores, H_min, H_max, tok_K_min=min(len(scores),6), tok_K_max=min(len(scores), topk))
    ...
    def adjust_tok_K(self,cache_number, H, H_min, H_max, tok_K_min=6, tok_K_max=15, a=1, b=None):
        ...
        H_tensor = torch.tensor(H)  # Convert H to a PyTorch tensor
        H_normalized = (H_tensor - H_min) / (H_max - H_min)
        #default
        liner_value = torch.sigmoid(a * (H_tensor - b))

        #sigmoid method
        #sigmoid_value = torch.sigmoid(H_normalized*a)
        
        #liner_value = (H - H_max)*10+1
        tok_K_new = tok_K_min + (tok_K_max - tok_K_min) * liner_value
        ...
        else: # If it's already an int, just round it
            if math.isinf(tok_K_new):
                tok_K_new = tok_K_min  # Fallback to minimum if infinity
            else:
                tok_K_new = round(tok_K_new)
        tok_K_new = min(tok_K_new, tok_K_max)  # Ensure it does not exceed the maximum value
        tok_K_new = max(tok_K_new, tok_K_min)  # Ensure it
...
#design for activate
    def activate(self, h):
        ...
        if self.h_var is None:
            ...
            loss, update_idx, dev_  = self.punish_percent_k(h, log=self.h_count%3200==0)
        ...
        data_to_save = { 
            'h_shape': h.size(),
            'self.h_var':self.h_var.item(),
            'self.h_mean':self.h_mean.item(),
            'self.h_count':self.h_count,
            'loss':loss.item(),
            #'acitvated_list':update_idx.tolist(),
            #'dev_':dev_.tolist(),
            'self.thr_v':self.thr_v.item()}
    ...
    def punish_percent_k(self, h, log=False):
        # calculate ms score
        #dev_g = (h - self.h_mean).pow(2) / (self.h_var + self.h_var.mean() / 100)
        ...
        # if str(self.thr_v.device) == 'cuda:0':
        #     print("thr:", self.thr_v.item(), "max dev:", dev_.max().item(), "mean", self.h_mean.mean().item(), "var", self.h_var.mean().item(), "percent", n_filtered.item()/(64.*2048*1024))
    
        emel_loss = torch.sum(torch.log(dev_g * update_idx + torch.ones_like(dev_)*(1-update_idx))) # /n_filtered
        # print("inhibited:", dev_.nelement()-torch.sum(update_idx), "before:", self.thr_v)
        ...
        # print("after:", self.thr_v)
        return emel_loss, update_idx, dev_
    ...
    def save_cached_blocks(self, block_score, block_k):
        ...
        data_to_save = {
            'layer_id': attention_num % 32,  # layer_id is self.inference_step modulo 32
