...
def predict_current_budget(current_score, n, means, variances, sample_sizes, samples, current_remaining_budget, tok_K_max, tok_K_min, k=100):
    ...
    with torch.no_grad():
        ...
        if total_samples == len(samples) * k:
            ...
            total_expected_score = expected_scores[n+1:].sum()

            # 计算当前玩家的权重
            weight_n = current_score / (current_score + total_expected_score)
        else:
            # 样本数量未达到要求，平均分配权重
...
class VectorTensor:
    ...
    def get_topk(self, tensor: torch.Tensor, avctivated_weight:torch.Tensor, topk):  # inner product
        ...
        if scores.numel() == 0:
            ...
            entropy_scores = torch.tensor(1e+5, dtype=scores.dtype, device=scores.device)
        else:
            normalized_scores = (scores - scores.min()) / (scores.max() - scores.min())
            normalized_scores = torch.softmax(normalized_scores, dim=0)
            entropy_scores = -torch.sum(normalized_scores * torch.log(normalized_scores))
        ...
        new_topk = self.adjust_tok_K(
            ...
            H_min,
            H_max,
            tok_K_min=min(len(scores), 2),
            tok_K_max=min(len(scores), topk)
        )

        #loss = self.activate(tensor)

        return logits.topk(new_topk, dim=0).indices.cpu().tolist(), logits.cpu().tolist()
...
class ContextManager:
    ...
    def calc_block_topk(
        ...
        if not self._use_chunk_topk:
            ...
            global_h_q_multihead = global_h_q
            ...
            for u in range(self.num_units):
                ...
                dev_ = self.activate(global_h_q_multihead[u])
                dev_ = dev_ / dev_.mean(dim=0, keepdim=True)
                dev_ = dev_.view(-1)
                #dev_ = dev_.squeeze(1)
