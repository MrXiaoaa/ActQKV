...
class VectorTensor:
    ...
    def adjust_tok_K(self, H, H_min, H_max, tok_K_min=2, tok_K_max=15, a=1, b=None):
        if b is None:
            b = (H_min + H_max) / 2
        ...
        sigmoid_value = torch.sigmoid(a * (H_tensor - b))
        tok_K_new = tok_K_min + (tok_K_max - tok_K_min) * sigmoid_value
    ...
    def calc_block_topk(
        ...
        if not self._use_chunk_topk:
            ...
            for u in range(self.num_units):
                update_idx = self.activate(global_h_q[u])
                global_h_q[u] = global_h_q[u]*update_idx[u]
