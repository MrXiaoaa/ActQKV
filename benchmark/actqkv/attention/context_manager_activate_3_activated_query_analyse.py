...
total_samples = [[] for _ in range(layer_num)]

total_mean_score = [0] * layer_num
layer_mean_score = 0
total_variance = [0] * layer_num
layer_variance = 0
total_sample_size = [0] * layer_num

overall_mean, overall_variance = 0, 0
...
global_h_upper = 1000
# top-k percent
global_activate_k = 0.02



def predict_current_budget(current_score, n, means, variances, sample_sizes, samples, current_remaining_budget, tok_K_max, tok_K_min, k=100):
    global layer_num, inference_step
    """
    优化后的预测当前玩家的奖金分配和剩余奖金的函数，防止显存泄漏。

    参数：
    - current_score: float 或标量，当前玩家的得分
    - n: int，当前玩家的索引（从0开始）
    - means: list 或标量，所有玩家的平均分列表
    - variances: list 或标量，所有玩家的方差列表
    - sample_sizes: list 或标量，所有玩家的样本数量列表
    - samples: list of lists，所有玩家的历史得分列表
    - current_remaining_budget: float，当前可分配的奖金池
    - tok_K_max: int，奖金分配的最大值
    - tok_K_min: int，奖金分配的最小值
    - k: int，保留的最大样本数量（默认值为100）

    返回：
    - current_budget: float，当前玩家的奖金
    - remaining_budget: float，剩余的奖金
    - updated_means: list，更新后的所有玩家的均值列表
    - updated_variances: list，更新后的所有玩家的方差列表
    - updated_sample_sizes: list，更新后的所有玩家的样本数量列表
    - samples: list of lists，更新后的所有玩家的历史得分列表
    - overall_mean: float，更新后的综合均值
    - overall_variance: float，更新后的综合方差
    - SSW: float，组内平方和
    - SSB: float，组间平方和
    """
    ...
    with torch.no_grad():
        ...
        # 将输入转换为张量，避免重复分配
        current_remaining_budget = torch.tensor(current_remaining_budget, dtype=dtype, device=device)

        # 更新当前玩家的样本
        samples[n].append(current_score.item())

        # 只保留每个玩家的最近k个样本
        for i in range(len(samples)):
            if len(samples[i]) > k:
                samples[i] = samples[i][-k:]  # 保留最后k个样本

        num_players = len(samples)

        # 预分配张量以减少内存分配次数
        updated_sample_sizes = torch.empty(num_players, dtype=dtype, device=device)
        updated_means = torch.empty(num_players, dtype=dtype, device=device)
        updated_variances = torch.empty(num_players, dtype=dtype, device=device)

        # 计算每个玩家的均值和方差
        for i in range(num_players):
            player_samples = samples[i]
            sample_size = len(player_samples)
            updated_sample_sizes[i] = sample_size
            if sample_size > 0:
                player_tensor = torch.as_tensor(player_samples, dtype=dtype, device=device)
                updated_means[i] = player_tensor.mean()
                # 使用无偏估计，如��样本数量小于2，���方差设为0
                if sample_size > 1:
                    updated_variances[i] = player_tensor.var(unbiased=True)
                else:
                    updated_variances[i] = torch.tensor(0.0, dtype=dtype, device=device)
                
                # 删除临时张量以释放显存
                del player_tensor
            else:
                updated_means[i] = torch.tensor(0.0, dtype=dtype, device=device)
                updated_variances[i] = torch.tensor(0.0, dtype=dtype, device=device)

        # 计算总体均值和方差
        N = updated_sample_sizes.sum()
        weighted_means = updated_means * updated_sample_sizes
        overall_mean = weighted_means.sum() / N

        # 计算组内平方和（SSW）
        SSW = ((updated_sample_sizes - 1) * updated_variances).sum()

        # 计算组间平方和（SSB）
        SSB = (updated_sample_sizes * (updated_means - overall_mean) ** 2).sum()

        # 计算总体方差
        if N > 1:
            overall_variance = (SSW + SSB) / (N - 1)
        else:
            overall_variance = torch.tensor(0.0, dtype=dtype, device=device)

        # 判断是否所有玩家的样本数量都达到k
        total_samples = updated_sample_sizes.sum().item()
        if total_samples == len(samples) * k and inference_step > layer_num:
        #if total_samples == len(samples) * k:

            # 所有玩家的样本数量都达到k，使用 total_expected_score 计算
            # 更新其他未揭示得分玩家的期望得分
            expected_scores = updated_means.clone()
            # for i in range(n + 1, num_players):
            #     mu_i = updated_means[i]
            #     tau_i_squared = updated_variances[i]
            #     s_n = updated_means[n]
            #     sigma_squared = overall_variance
            #     # 避免除以零，增加一个小的偏置项1e-8
            #     denominator = sigma_squared + tau_i_squared + 1e-8
            #     expected_scores[i] = (sigma_squared * mu_i + tau_i_squared * s_n) / denominator

            # 计算总期望得分（只考虑当前玩家和剩余玩家）
            total_expected_score = expected_scores[n:].sum()

            # 计算当前玩家的权重
            weight_n = expected_scores[n] / total_expected_score
        else:
            # 样本数量未达到要求，平均分配权重
            num_remaining_players = num_players - n
            weight_n = 1.0 / num_remaining_players

        # 计算当前玩家的奖金
        current_budget = weight_n * current_remaining_budget

        # 确保 tok_K_new 是一个有效的数值
        if isinstance(current_budget, torch.Tensor):
            if torch.isnan(current_budget):
                tok_K_new = tok_K_min  # 如果 NaN，则回退到最小值
            else:
                tok_K_new = torch.round(current_budget).item()  # 四舍五入并转换为 Python float
        else:
            if math.isinf(current_budget) or math.isnan(current_budget):
                tok_K_new = tok_K_min  # 如果是无穷大或 NaN，回退到最小值
            else:
                tok_K_new = round(current_budget)  # 如果已经是 float，直接四舍五入

        tok_K_new = min(tok_K_new, tok_K_max)  # 确保不超过最大值
        tok_K_new = max(tok_K_new, tok_K_min)  # 确保不低于最小值

        # 计算剩余奖金
        remaining_budget = current_remaining_budget.item() - tok_K_new

        return (
            int(tok_K_new),
            remaining_budget,
            updated_means.tolist(),
            updated_variances.tolist(),
            updated_sample_sizes.tolist(),
            samples,
            overall_mean.item(),
            overall_variance.item(),
            SSW.item(),
            SSB.item()
        )
...
class VectorTensor:
    ...
    def get_topk(self, tensor: torch.Tensor, avctivated_weight:torch.Tensor, activate_question_weight:Optional[torch.Tensor],topk):  # inner product
        
        global inference_step, layer_num
        original_tensor = tensor
        #assert avctivated_weight.dim() == 1 and avctivated_weight.size(0) == self.hidden_size, f"Error: activated_weight size {avctivated_weight.size()} does not match tensor size {tensor.size()}"
        #logits_c = torch.matmul(self.data[:self.length], tensor[:, None]).squeeze(dim=-1)
        tensor = (avctivated_weight * tensor).sum(dim=0, keepdim=False)
        #tensor = tensor * avctivated_weight.sum() / avctivated_weight.size(0)
        tensor = tensor.to(self.data.dtype)  # 转换 tensor 的 dtype 以匹配 self.data
        assert tensor.dim() == 1 and tensor.size(0) == self.hidden_size
        # dot product
        #logits_c = torch.matmul(self.data[:self.length], tensor[:, None]).squeeze(dim=-1) 
        # cosine similarity
        logits_c = torch.matmul(F.normalize(self.data[:self.length], dim=-1), F.normalize(tensor, dim=-1)[:, None]).squeeze(dim=-1) 
        ##################################################################
        # change here
        if self.question is not None:
            # avctivated_weight_mean = avctivated_weight.mean()
            # self.question = self.question * avctivated_weight_mean

            #logits_q = torch.matmul(self.data[:self.length], self.question[:, None]).squeeze(dim=-1)
            logits_q = torch.matmul(F.normalize(self.data[:self.length], dim=-1), F.normalize(self.question, dim=-1)[:, None]).squeeze(dim=-1)
        ...
        # avctivated_tensor = tensor * avctivated_weight
        # avctivated_tensor = avctivated_tensor.to(self.data.dtype)  # 转换 tensor 的 dtype 以匹配 self.data
        # activated_logits_c = torch.matmul(self.data[:self.length], avctivated_tensor[:, None]).squeeze(dim=-1)
        # # Compute scores on GPU and preserve type and device
        if inference_step > layer_num:
            scores = logits_c # Keep scores on GPU as a tensor
        else:
            scores = logits

        normalized_scores = F.softmax(scores, dim=0)
        #normalized_scores = scores / (scores.sum() + 1e-10)  # Prevent division by zero

        entropy_scores = -torch.sum(normalized_scores * torch.log(normalized_scores + 1e-10))
        
        # Define H_min and H_max on GPU
        H_min = torch.tensor(0.0, device=scores.device, dtype=scores.dtype)
        H_max = torch.log(torch.tensor(len(scores) + 1e-10, device=scores.device, dtype=scores.dtype))
        
        #Optionally, move entropy_scores to CPU if adjust_tok_K requires a Python float
        if activate_question_weight is not None and self.data[:self.length].size(0)==190:
            # dev_ = ContextManager.activate(self.full_question)
            # dev_ = dev_.mean(dim=1, keepdim=True)
            # dev_ = dev_ / dev_.sum()
            activated_question = (activate_question_weight * self.full_question)

            data_to_save = {
                'layer_id': attention_num % layer_num,
                'inference_token_id': inference_step // layer_num,
                'attention_num': attention_num,
                'inference_step': inference_step,
                'original_tensor_shape': original_tensor.size(),
                'activated_tensor_shape': tensor.size(),
                'full_question_shape': self.full_question.size(),
                'KV_shape':self.data[:self.length].size(),
                'avctivated_weight_shape': avctivated_weight.size(),
                'activate_question_weight_shape': activate_question_weight.size(),
                'activate_question_weight': activate_question_weight.cpu().tolist(),
                'original_tensor': original_tensor.cpu().tolist(),
                'activated_tensor': (avctivated_weight * original_tensor).cpu().tolist(),
                'KV':self.data[:self.length].cpu().tolist(),
                'full_question': self.full_question.cpu().tolist(),
                'activated_question': activated_question.cpu().tolist(),
                #'entropy_scores': entropy_scores.cpu().tolist(),
                #'logits_c': logits_c.cpu().tolist(),
                #'activated_logits_c': activated_logits_c.cpu().tolist(),
                #'avctivated_weight': avctivated_weight.cpu().tolist(),
            }       
            file_name = f'./result/score/activate_full-layer-3.json'

            with open(file_name, 'a') as f:
                json.dump(data_to_save, f)
                f.write('\n')

        #print('scores: ', scores, 'entropy_scores: ', entropy_scores)
        new_topk = self.adjust_tok_K(
            scores.size(0),
            entropy_scores,
            H_min,
            H_max,
            tok_K_min=min(len(scores), 32),
            tok_K_max=min(len(scores), topk))

        #loss = self.activate(tensor)

        return scores.topk(new_topk, dim=0).indices.cpu().tolist(), scores.cpu().tolist()
    ...
    def set_question(self, question):
        ...
        assert self.question.dim() == 1 and self.question.size(0) == self.hidden_size, self.question.shape

    def set_full_question(self, question):
        assert self.full_question is None
        self.full_question = question

    def __len__(self):
        return self.length

    def adjust_tok_K(self, cache_number, H, H_min, H_max, tok_K_min=6, tok_K_max=15, a=1, b=None):
        global remaining_budget, total_mean_score, total_variance, total_sample_size, overall_mean, overall_variance, total_samples
       
        if b is None:
            b = (H_min + H_max) / 2
        
        # Check for valid H_max and H_min to avoid division by zero
        if H_max == H_min:
            return tok_K_min  # Return minimum if no variation in H
        
        #best
        H_normalized = (H - H_min) / (H_max - H_min)
        #H_normalized = torch.sigmoid((H - H_min) / (H_max - H_min))

        

        # Default
        tok_K_new, remaining_budget, total_mean_score, total_variance, total_sample_size, total_samples, overall_mean, overall_variance, SSW, SSB = predict_current_budget(
            H_normalized, attention_num % layer_num, total_mean_score, total_variance, total_sample_size, total_samples, token_budget, tok_K_max, tok_K_min, warmup_step)

        #total_budgets[attention_num % layer_num] = remaining_budget
        # Check for NaN and handle it
        if math.isnan(tok_K_new):
            print(f"Remaining Budget: {remaining_budget}")
            print(f"Total Mean Score: {total_mean_score}")
            print(f"Total Variance: {total_variance}")
            print(f"Total Sample Size: {total_sample_size}")
            print(f"Overall Mean: {overall_mean}")
            print(f"Overall Variance: {overall_variance}")
            print(f"Layer ID: {attention_num % layer_num}")
            print(f"Inference Token ID: {inference_step // layer_num}")
            print(f"Cache Number: {cache_number}")
            print(f"Origin Topk: {tok_K_max}")
            print(f"New Topk: {tok_K_new}")
        ...
        data_to_save = {
            ...
            'origin_topk':tok_K_max,
            'new_topk': tok_K_new,
            'token_budget': token_budget,
            'remaining_budget': remaining_budget,
            'H':H.item(),
            'H_max':H_max.item(),
            'H_normalized': H_normalized.item(),
            'layer_mean_score': total_mean_score[attention_num % layer_num],
            'layer_variance': total_variance[attention_num % layer_num],
        ...
        file_name = f'./result/score/longbench_llama3_cached_blocks_scores_dynamic_test.json'

        # #if attention_num % layer_num == 0:
        # with open(file_name, 'a') as f:
        #     json.dump(data_to_save, f)
        #     f.write('\n')
        return tok_K_new
    # def adjust_tok_K(self, cache_number, H, H_min, H_max, tok_K_min=2, tok_K_max=15, a=20, b=0.8):
    #     global attention_num, inference_step
    ...
    #     H_tensor = torch.tensor(float(H), dtype=torch.float32)  # Ensure H is a Python float
    #     H_max_tensor = torch.tensor(float(H_max), dtype=torch.float32)
    #     sigmoid_value = torch.sigmoid((H_tensor - H_max_tensor) * 10) * 2
    #     sigmoid_value = torch.exp(10 * (sigmoid_value - 1))  # e的10*(sigmoid_value-1)次方
    #     tok_K_new = tok_K_min + (tok_K_max - tok_K_min) * sigmoid_value
        
    #     # Ensure tok_K_new is a valid number before rounding
    #     if torch.isnan(tok_K_new):
    #         tok_K_new = tok_K_min  # Fallback to minimum if NaN
        
    #     # Ensure tok_K_new is a tensor before calling .item()
    #     if isinstance(tok_K_new, torch.Tensor):
    #         tok_K_new = round(tok_K_new.item())  # Convert back to a Python float and round
    #     else:
    #         tok_K_new = round(tok_K_new)  # If it's already an int, just round it

    #     tok_K_new = min(tok_K_new, tok_K_max)  # Ensure it does not exceed the maximum value
    #     tok_K_new = max(tok_K_new, tok_K_min)  # Ensure it does not fall below the minimum value
        
    #     data_to_save = {
    #         'layer_id': attention_num % 32,
    #         'inference_token_id': inference_step,
    #         'cache_number':cache_number,
    #         'origin_topk':tok_K_max,
    #         'new_topk': tok_K_new,
    #         'block_entropy':H,
    #         'block_max_entropy':H_max,
    #         'normalize_entropy':H-H_max,
    #         'sigmoid_input':((H - H_max)*10+1),
    #         'sigmoid_value':sigmoid_value.item(),
    #         'tok_K_max':tok_K_max,
    #         'tok_K_min':tok_K_min

    #     }

    #     file_name = f'./result/score/longbench_llama3_cached_blocks_scores_dynamic.json'
...
#         ##################################################################
#         # add q
#         logits_c = tensor[None, :].cpu().float().numpy().astype("float32")
#         logits_q = torch.matmul(self.data[:self.length], self.question[:, None])
#         logits = logits_c + logits_q
#         topk_index = self.index.search(logits, topk)[1][0].tolist()
#         return topk_index

#     def set_question(self, question):
#         assert question.shape[-1] == self.hidden_size
#         self.question = question
...
class ContextManager:
    def __init__(self, 
        ...
        self.question_weight = question_weight
        # global global_h_var, global_h_mean, global_thr_v, global_h_count, global_h_upper, global_activate_k
        # #design for activate
        # global_h_var = None
        # global_h_mean = None
        # # 维护阈值
    ...
    def init(
        ...
        if self.faiss:
            ...
            self.block_k = [VectorTensor(
                dim_head * self.unit_size, global_k.dtype, question_weight=self.question_weight,
            ) for _ in range(self.num_units)]
    ...
    def calc_block_topk(
        ...
        if not self._use_chunk_topk:
            ...
            a, b, c, d = global_h_q.size()
            global_h_q = global_h_q.permute(0, 2, 1, 3).reshape(a, c, b * d)
            ...
            for u in range(self.num_units):
                #print("global_h_q[u].shape:", global_h_q[u].shape)
                assert global_h_q[u].shape == (c, self.unit_size * self.dim_head)
                
        
                dev_ = self.activate(global_h_q[u])
                dev_ = dev_.mean(dim=1, keepdim=True)
                dev_ = dev_ / dev_.sum()
                #query = (dev_ * global_h_q[u]).sum(dim=0, keepdim=False)
                if self.full_question is not None:
                    _, activate_question_weight = self.punish_percent_k(self.full_question[u])
                    activate_question_weight = activate_question_weight.mean(dim=1, keepdim=True)
                    activate_question_weight = activate_question_weight / activate_question_weight.sum()
                    topk, score = self.block_k[u].get_topk(
                        global_h_q[u], dev_, activate_question_weight, self.topk if self.topk < self.num_global_block else self.num_global_block)
                else:
                    topk, score = self.block_k[u].get_topk(
                        global_h_q[u], dev_, None, self.topk if self.topk < self.num_global_block else self.num_global_block)
               
                #dev_ = dev_.squeeze(1)
                # global_h_q[u] = global_h_q[u]*dev_[u]
                #dev_q = self.activate(self.question)
                
                #separate activate and score update
                #global_h_q[u] = global_h_q[u]*dev_[u]
                ret.append(topk)
...
#design for activate
    def activate(self, h):
        ...
        dtype_ = h.dtype
        device_ = h.device
        # loss = torch.tensor(0.0,  dtype=dtype_)
        update_idx = torch.ones(h.size(0), h.size(1), dtype=dtype_, device=device_)
        dev_ = torch.ones(h.size(0), h.size(1), dtype=dtype_, device=device_)
        h_ = torch.tensor(h.clone().detach(), dtype=torch.float32)
        #h_ = h.clone().detach().type(torch.float32)
        # 当前层的均值和方差
        ...
        # 更新统计量
        if global_h_var is None:

            global_h_var = h_v / (_batch_size - 1.) * _batch_size
            global_h_mean = h_u
            dev = ((h_ - global_h_mean).pow(2) / (global_h_var + global_h_var.mean() / 100)).reshape(-1)
            n_ele = dev.shape[0]
            n_inh = int(n_ele*global_activate_k)
            global_thr_v = -torch.kthvalue(-dev, n_inh, keepdim=True)[0]
            
        elif global_h_count <= global_h_upper:
            global_h_var = (global_h_var * (global_h_count - 1. / _batch_size) + h_v +
                            (h_u - global_h_mean) ** 2 / (1 + 1. / global_h_count)) / \
                            (global_h_count + 1 - 1. / _batch_size)
            global_h_mean = (global_h_mean * global_h_count + h_u) / (global_h_count + 1.)
            
            dev = ((h_ - global_h_mean).pow(2) / (global_h_var + global_h_var.mean() / 100)).reshape(-1)
            n_ele = dev.shape[0]
            n_inh = int(n_ele*global_activate_k)
            global_thr_v = (global_thr_v * global_h_count - torch.kthvalue(-dev, n_inh, keepdim=True)[0]) / (global_h_count + 1.)
            
        else:
            global_h_var = (global_h_var * (global_h_upper - 1. / _batch_size) + h_v +
                            (h_u - global_h_mean) ** 2 / (1 + 1. / global_h_upper)) / \
                            (global_h_upper + 1 - 1. / _batch_size)
            global_h_mean = (global_h_mean * global_h_upper + h_u) / (global_h_upper + 1.)
            
            update_idx, dev_  = self.punish_percent_k(h, log=global_h_count%3200==0)

        global_h_count = global_h_count+1
        # 检查是否有非有限值
        if global_h_var.mean().isfinite() is False:
            raise ValueError('h_var is not finite')
            
        data_to_save = { 
            'h_shape': h.size(),
            # 'global_h_var':global_h_var.tolist(),
            # 'global_h_mean':global_h_mean.tolist(),
            'global_h_count':global_h_count,
            #'acitvated_list':update_idx.tolist(),
            # 'dev_':dev_.tolist(),
            # 'global_thr_v':global_thr_v.item()
            }
        file_name = f'./result/score/tensor_shape.json'
    ...
    def punish_percent_k(self, h, log=False):
        global global_h_var, global_h_mean, global_thr_v, global_h_count, global_h_upper, global_activate_k
        # calculate ms score
        #dev_g = (h - global_h_mean).pow(2) / (global_h_var + global_h_var.mean() / 100)
        #dev_ = dev_g.clone().detach()
        dev_g = (h - global_h_mean).pow(2)
        dev_ = dev_g.clone().detach() / (global_h_var + global_h_var.mean() / 100)
        update_idx = torch.where(dev_ >= global_thr_v, torch.ones_like(dev_), torch.zeros_like(dev_))
        
        n_filtered = torch.sum(update_idx)
        # if str(global_thr_v.device) == 'cuda:0':
        #     print("thr:", global_thr_v.item(), "max dev:", dev_.max().item(), "mean", global_h_mean.mean().item(), "var", global_h_var.mean().item(), "percent", n_filtered.item()/(64.*2048*1024))
    
        #emel_loss = torch.sum(torch.log(dev_g * update_idx + torch.ones_like(dev_)*(1-update_idx))) # /n_filtered
        # print("inhibited:", dev_.nelement()-torch.sum(update_idx), "before:", global_thr_v)
        global_thr_v = global_thr_v - (int(dev_.nelement()*global_activate_k)-n_filtered)/dev_.nelement()
        # if log and str(h.device) == 'cuda:0':
        #     print("thr:" + str(global_thr_v)+", filtered/to filter: "+str(n_filtered)+"/"+str(int(dev_.nelement()*global_activate_k))+", MS:"+str(torch.sum(dev_*update_idx).item()))
        # print("after:", global_thr_v)
        return update_idx, dev_
    ...
    def get_global_hidden_and_mask(
        ...
        assert len(block_topk) == self.num_units
        global_block_map = [[-1] * self.topk for _ in range(self.num_units)]  # Initialize with -1
        global_remainder_len = max(self._global_remainder_ed - self._global_remainder_st + len_q - self.n_local, 0)
        init_len = self.init_k.size(-2)
        sliding_window = None
        ...
        for u in range(self.num_units):
            # assert max(global_block_map[u][block_num:] + [-1]) == -1
            # assert min(global_block_map[u][:block_num] + [0]) > -1
            global_block_map[u] = list(global_block_map[u][:block_num]) + [-1] * (self.topk - block_num)  # Ensure the remaining elements are -1
    ...
    def _append(
        ...
        return o.view((self.batch_size, self.num_heads, -1, self.dim_head)), loc_score, block_score, block_topk
    ...
    def append_global(
        ...
        if not self.init_exc and global_remainder_len > self.n_local:
            ...
            #################################################################################################
            # only set question for the first time
            if self.question is None and self.question_ids is not None and self.question_ids[1] - self.question_ids[0] > 0:
                ...
                num_questions = question.size(2)  # Calculate the exact size
                self.full_question = question.permute(0, 2, 1, 3).reshape(self.num_units, num_questions, self.dim_head * self.unit_size)
                if question.size(2) > 0:
                    ...
                    for u in range(self.num_units):
                        ...
                        self.block_k[u].set_full_question(self.full_question[u])
    ...
    def append(
        ...
        #init total_budgets according to length of the input 
        global total_budgets, token_budget, attention_num, inference_step
        if attention_num % layer_num == 0:
            budget_size = self.topk * layer_num /2
            budget_num = math.ceil(input_length / self.exc_block_size)
            total_budgets = [budget_size] * budget_num

        for iteration_count, st in enumerate(range(0, input_length, self.exc_block_size)):
            #get the budget for the current iteration
            token_budget = total_budgets[iteration_count]
            ...
            #update the budget for the current iteration
            total_budgets[iteration_count] = remaining_budget
        ...
        #mad_result = self.calculate_mad(ret)
        

        # with open(f'./result/score/exp_score_lllm.jsonl', 'a') as f:
        #     json.dump({'attention_num': attention_num % 32, 'block_score': block_score, 'ret_shape': ret.size(), 'block_k': block_k, 'chunk_num': attention_num // 32 }, f)
        #     f.write('\n')
        
        
        
        #self.save_cached_blocks(block_score, block_k)
        attention_num += 1
        inference_step += 1
        return ret
    

    # def get_batched_topk_dynamic(self, global_q):
    #     length = global_q.shape[2]
    #     exc_num = (length + self.exc_block_size - 1) // self.exc_block_size
    #     exc_block_num = length // self.exc_block_size
    #     ret = []
    #     if self.num_global_block <= self.new_topk:
    #         for _ in range(exc_num):
    #             ret.append(
    #                 [list(range(len(self.global_blocks[0]))) for _ in range(self.num_units)]
    #         return ret
    ...
    #         # Adjust new_topk based on block_score entropy
    #         self.entropy = entropy(block_score.cpu().numpy()).mean()
    #         H_min = 0
    #         H_max = math.log(block_score.size(-1))
    #         self.new_topk = self.adjust_tok_K(self.entropy, H_min, H_max, tok_K_min=2, tok_K_max=min(block_score.size(-1), self.topk))

    #         indices = block_score.topk(self.new_topk, dim=-1).indices.cpu()
    #         for b in range(exc_block_num):
    #             tmp = []
    #             for u in range(self.num_units):
    #                 tmp.append(indices[u, b].tolist())
    #                 assert len(tmp[-1]) == self.new_topk
    ...
    #         self.score = block_score.cpu().numpy()
    #         # Adjust new_topk based on block_score entropy
    #         self.entropy = entropy(block_score.cpu().numpy()).mean()
    #         H_min = 0
    #         H_max = math.log(block_score.size(-1))
    ...
    # def adjust_tok_K(self, H, H_min, H_max, tok_K_min=5, tok_K_max=15, a=1, b=None):
    #     if b is None:
    #         b = (H_min + H_max) / 2
    #     H_normalized = (H - H_min) / (H_max - H_min)
    #     sigmoid_value = torch.sigmoid(a * (H - b))
    #     tok_K_new = tok_K_min + (tok_K_max - tok_K_min) * sigmoid_value
    #     tok_K_new = round(tok_K_new)
    #     tok_K_new = min(tok_K_new, tok_K_max)  # 确保不超过最大值
    #     tok_K_new = max(tok_K_new, tok_K_min)  # 确保不低于最小值
    #     return tok_K_new
    ...
    def save_cached_blocks(self, block_score, block_k):
        # Implement your saving logic here
        # For example, save to a list or write to a file
        data_to_save = {
            'layer_id': attention_num % layer_num,  # layer_id is self.inference_step modulo 32
            'inference_token_id': self.inference_step,  # inference_token_id is self.inference_step divided by 32
            'cached_blocks_number': len(self.cached_blocks),
            'selected_block_number': len(block_k[0])
        }
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        pid = os.getpid()
        file_name = f'./result/score/longbench_llama3_cached_blocks_scores_init{self.n_init}_local{self.n_local}_blocksize{self.block_size}_topk{self.topk}_pid{pid}_dynamic.json'
        with open(file_name, 'a') as f:
            json.dump(data_to_save, f)
            f.write('\n')
        self.inference_step += 1

    # def calculate_mad(self, ret):
    #     """
    #     在 input_length 维度上计算 MAD，并返回形状为 [batch_size, num_heads, mad] 的张量。
    #     """
    #     # 计算 input_length 维度的均值，保持维度以便后续计算
    #     mean = torch.mean(ret, dim=-2, keepdim=True)

    #     # 计算每个元素与均值的绝对差值
    #     abs_deviation = torch.abs(ret - mean)

    #     # 对 input_length 维度上的绝对差值求平均，得到 MAD
    #     mad = torch.mean(abs_deviation, dim=-2)

    #     # 返回结果，形状为 [batch_size, num_heads, mad]
    #     return mad